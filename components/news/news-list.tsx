  ...
  const filteredNews = selectedCategory
    ? mockNews.filter(news => {
        ...
        if (selectedCategory === "promotion") {
          return title.includes("khuyến mãi") || title.includes("giảm giá") || title.includes("tích điểm")
        }
        if (selectedCategory === "announcement") {
          return title.includes("thông báo") || title.includes("bảo trì")
        }
        if (selectedCategory === "news") {
          return title.includes("ra mắt") || title.includes("đạt chứng nhận")
        }
        return false
      })
    : mockNews
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {filteredNews.map((news) => (
          <Link key={news.newfeed_id} href={`/news/${news.newfeed_id}`}>
